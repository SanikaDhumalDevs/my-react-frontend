...
const AddEntry = () => {
  ...
  useEffect(() => {
    const fetchEmissionFactor = async () => {
      ...
      try {
        ...
        const response = await fetch(`http://localhost:5000/api/emission-factor/calculate?${queryParams}`);
 ...
 // OCR Upload to Python port 5001
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('http://localhost:5000/api/entries/add', {
        method: 'POST',
        body: form,
      });
