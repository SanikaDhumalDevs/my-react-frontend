...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchEntries = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/api/dashboard?email=${userEmail}`);
        ...
        if (
          ...
          await axios.post("http://localhost:5000/api/notify-expiry", {
            to: userEmail,
          });
  ...
  const handleDeleteExpiredEntries = async () => {
    ...
    try {
      const delRes = await axios.post(
        `http://localhost:5000/api/entries/delete-expired`,
        { email: userEmail }
      );
      ...
      if (delRes.status === 200) {
        ...
        try {
          const refreshed = await axios.get(
            `http://localhost:5000/api/dashboard?email=${encodeURIComponent(userEmail)}`
          );
